            ...
            if (step+1) % args.eval_every == 0:
                ...
                if mes >= best_mes:
                    ...
                    if torch.cuda.device_count() > 1:
                        ...
                        torch.save(model.state_dict(), args.save)
                print(step+1, avg_loss/len(log_prob_ps))
                avg_loss = 0.0
...
def train(args, model, loss_fn, m_optim, m_scheduler, metric, train_loader, dev_loader, device):
    best_mes = 0.0
    tb_writer = SummaryWriter(log_dir=args.log)
    for epoch in range(args.epoch):
        avg_loss = 0.0
        for step, train_batch in enumerate(train_loader):
            if args.model == 'bert':
            ...
            if args.task == 'ranking':
                ...
                score=torch.cat([batch_score_pos.unsqueeze(1),batch_score_neg.unsqueeze(1)],dim=1)
                
                loss= F.log_softmax(score, dim=1)#.mean()
            ...
            if (step+1) % args.eval_every == 0:
                ...
                tb_writer.add_scalar("Train/loss", avg_loss/args.eval_every, step)
